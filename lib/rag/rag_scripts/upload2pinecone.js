...
import { PineconeUtils, CONFIG as PINECONE_CONFIG } from '../../service/pinecone/index.ts';
import dotenv from 'dotenv';

// How to use this script:
// node lib/rag/rag_scripts/upload2pinecone.js <command> [args]
 ...
 * @returns {Promise<Object[]>} - JSON数据数组
 */
 ...
 * @param {number} index - 产品在数组中的索引
 * @returns {Object} - meta信息对象
 */
function extractMetaFromJson(product, index) {

    const meta = {
        // 基本信息
        productId: product.parent_asin|| 'Error : Missing Product ID',
        mainCategory: product.main_category || '',
        
        // 评分信息
        averageRating: product.average_rating || null,
        ratingNumber: product.rating_number || null,
        ...
        // 分类信息 - ensure it's an array of strings
        categories: Array.isArray(product.categories) ? product.categories.map(String) : [],
        
        // 特性信息 - ensure it's an array of strings
        features: Array.isArray(product.features) ? product.features.map(String) : [],
        
        // 描述信息 - ensure it's an array of strings
        description: Array.isArray(product.description) ? product.description.map(String) : [],
        
        // 详细信息 - convert object to JSON string
        details: product.details && typeof product.details === 'object' ? JSON.stringify(product.details) : '',
        
        // 父ASIN
        parentAsin: product.parent_asin || null,
        
        // 图片信息 - ensure it's an array of strings
 ...
 */
async function processSingleJsonProduct(product, index) {
    try {
        const productId = product.parent_asin || 'Error : Missing Product ID';
        const title = product.title || 'Unknown Product';
        const content = product.title || 'Error : Missing content'; 
        const meta = extractMetaFromJson(product, index);
        
        console.log(`🔄 Processing product ${productId} (${index + 1})...`);
        
        if (!content.trim()) {
            console.warn(`⚠️ Warning: Product ${productId} has empty content, skipping...`);
            return;
        }
        
        const embedding = await generateEmbedding(content);
        
        await PineconeUtils.uploadProduct(productId, title, content, embedding, meta);
    } catch (error) {
        console.error(`❌ Failed to process product ${index + 1}:`, error.message);
        throw error;
    }
}

/**
 * 批量处理JSON产品数据并上传到Pinecone
 * @param {string} jsonFilePath - JSON文件路径
 * @param {number} batchSize - 批处理大小
 */
async function processAndUploadProductsFromJson(jsonFilePath, batchSize = CONFIG.BATCH_SIZE) {
    try {
        console.log('🚀 Starting JSON product upload process...');
        ...
        for (let i = 0; i < products.length; i += batchSize) {
            ...
            const results = await Promise.allSettled(
                batch.map((product, batchIndex) => processSingleJsonProduct(product, i + batchIndex))
            );
            ...
            results.forEach((result, batchIndex) => {
                if (result.status === 'fulfilled') {
                    ...
                    console.error(`❌ Batch ${batchNum}, product ${globalIndex}: ${result.reason.message}`);
                }
            });
        ...
        console.log(`\n🎉 JSON product upload completed!`);
        console.log(`✅ Success: ${stats.success}, ❌ Failed: ${stats.failed}`);
        
    } catch (error) {
        console.error('❌ Error processing JSON products:', error.message);
        throw error;
    }
}
...
class CLIHandler {
    static showUsage() {
        console.log('📖 Usage:');
        console.log('  Upload Products new:  node upload2pinecone.js uploadProductsN <product-json-file> [batch-size]');
    ...
    static async handleUploadProductsN(args) {
        if (args.length < 2) {
            console.error('❌ Please provide product JSON file path');
            process.exit(1);
        }
        
        const productJsonFile = args[1];
        ...
        return processAndUploadProductsFromJson(productJsonFile, batchSize);
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    const handlers = {
        'uploadProductsN': CLIHandler.handleUploadProductsN,
...
export {
    ...
    processAndUploadProductsFromJson,
    processAndUploadComments,
    queryProducts,
    
    // JSON处理函数
    readJsonFile,
    processSingleJsonProduct,
...
// 向后兼容的函数别名
export const uploadProductToPinecone = PineconeUtils.uploadProduct;
export const uploadCommentsToPinecone = PineconeUtils.uploadComments;
export const extractCommentParentAsin = ContentExtractor.extractCommentParentAsin;
export const extractCommentBatch = ContentExtractor.extractCommentBatch;
export const extractCommentCount = ContentExtractor.extractCommentCount;